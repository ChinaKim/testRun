...
public class ZoomTutorial {
    ...
    public boolean getScaleFinalBounds(int position) {
        //得到显示区域中第一个子视图的序号
        LinearLayoutManager layoutManager = (LinearLayoutManager) ((RecyclerView) mThumbViewParent).getLayoutManager();
        ...
        mContainView.findViewById(R.id.fra_cer_view).getGlobalVisibleRect(finalBounds, globalOffset);
        startBounds.offset(-globalOffset.x, -globalOffset.y);
        finalBounds.offset(-globalOffset.x, -globalOffset.y);
        //设置比率
        set_Center_crop(finalBounds);
