...
public class RoundProgressBar extends View {
    ...
    public RoundProgressBar(Context context, AttributeSet attrs, int defStyle) {
        ...
        radius = mTypedArray.getDimension(
                R.styleable.RoundProgressBar_radiusSize, 5);
//        max = mTypedArray.getInteger(R.styleable.RoundProgressBar_max, 100);
    ...
    protected void onDraw(Canvas canvas) {
        super.onDraw(canvas);
        float centrew = getWidth() / 2; // 获取圆心的x坐标
        float centreh = getHeight() / 2;
        // paint.setAlpha(0);
        ...
        //绘制底层背景
        tempcCanvas.drawColor(Color.parseColor("#A0FFFFFF"));
        //绘制圆
        tempcCanvas.drawCircle(centrew, centreh, radius, paint);
        RectF oval = new RectF(centrew - radius, centreh - radius, centrew + radius, centreh + radius);
        // 用于定义的圆弧的形状和大小的界限
        paint.setStrokeWidth(0);
        paint.setStyle(Paint.Style.FILL_AND_STROKE);
        paint.setAlpha(0);
        tempcCanvas.drawArc(oval, -90, 360 * progress / max, true, paint);
